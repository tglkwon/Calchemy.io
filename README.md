...
### **1.3. 맵 시스템 (Map System)**

게임은 **Slay the Spire** 스타일의 맵 진행 시스템을 채택합니다.

* **맵 생성:** 절차적(Procedural) 생성으로 매 플레이마다 다른 구조의 맵 제공.
* **노드 타입:**
  * ⚔️ **전투 (BATTLE):** 몬스터와의 전투. 승리 시 보상 획득.
  * 🛒 **상점 (SHOP):** 골드를 사용해 카드/유물 구매 (예정).
  * 💤 **휴식 (REST):** 체력 회복 또는 카드 업그레이드 선택 (예정).
  * ❓ **이벤트 (EVENT):** 랜덤 텍스트 이벤트 발생. 선택지에 따라 보상(골드, 유물 등)이나 패널티(체력 감소)가 주어짐. (구현 완료)
  * 💎 **보물 (TREASURE):** 유물 획득 (예정).
  * 👑 **보스 (BOSS):** 층의 마지막 전투. 클리어 시 다음 층으로 이동 (예정).
* **진행 방식:** 현재 노드에서 연결된(도달 가능한) 노드만 선택 가능. 방문한 노드는 시각적으로 표시됨.
...
* **중앙 (Field):**  
  * **상단:** 턴 카운터, 메시지 로그. (유물 정보는 Global Status Bar로 이동)  
  * **중앙:** 4x4 카드 그리드 (빙고판).  
  * **하단:** 제어 버튼 (시작, 일시정지, 재시작).  
* **우측 (Enemy):** 적 유닛(최대 3마리), 각각의 HP/의도(Intent) 아이콘 표시.

### **4.2. Global Status Bar (신규)**

모든 페이지 상단(NavBar 하단)에 고정되어 플레이어의 핵심 정보를 표시합니다.

*   **HP:** 현재 체력 / 최대 체력
*   **Gold:** 보유 골드량
*   **Potions:** 보유한 포션 슬롯 (최대 3개). 클릭 시 사용 가능.
*   **Relics:** 활성화된 유물 아이콘 목록. 마우스 오버 시 툴팁 제공.

### **4.3. 보상 페이지 (Reward Page) (신규)**

전투 승리 시 진입하는 전용 화면으로, 플레이어의 성취감을 고취시킵니다.

*   **골드 보상:** 몬스터 등급에 따른 골드 (클릭 시 획득).
*   **카드 드래프트:** 3장의 카드 중 1장을 선택하거나 건너뛰는(Skip) 전략적 선택.
*   **유물 선택:** 엘리트/보스 처치 시 유물 선택지 제공.
...
진행 중 만나는 랜덤 서사 이벤트입니다.

* **파일:** events.csv
* **주요 데이터:**
  * **EventName:** 이벤트 제목
  * **Summary:** 이벤트 설명문
  * **Choices:** 선택지 목록 (구분자 `/`, `또는` 등으로 분리).
  * **Effect:** 선택 시 발동할 효과 (골드 획득, 체력 회복, 카드 제거 등).
