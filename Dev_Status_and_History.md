...
*   **휴식(REST) 로직 통합 (Rest Logic Integration):**
    *   기존의 휴식 노드는 독립된 기능을 수행하는 대신, 게임의 전략적 선택지를 집중시키기 위해 상점(SHOP) 노드로 완전히 통합되었습니다.
    *   **노드 구성 변경:** `MapGenerator.js`의 노드 생성 알고리즘이 수정되어, 기존의 `RoomType.REST` 위치에는 이제 `RoomType.SHOP`이 배치됩니다.
    *   **기능 이전:**
        *   **체력 회복:** 상점에서 판매되는 포션이나 특정 유물을 통해 수행합니다.
        *   **카드 정비:** 상점 내 카드 제거 및 카드 강화 서비스를 통해 덱 압축과 밸런싱을 동시에 관리합니다.
    *   **전략적 의도:** 플레이어는 단순히 '쉬는 것'이 아니라, 획득한 골드를 어떻게 투자할지 상점에서 더 깊은 고민을 하게 됩니다.
